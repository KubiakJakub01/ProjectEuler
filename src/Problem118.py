...
def solution():
    ...
    for i, p in enumerate(prime):
        availability[p] = [x for x in prime[i:] if len(set(p).intersection(set(x)))==0]
    prime.clear()
...
def findingPandigitalPrimeSets(value, curr_set, intersection):
    ...
    for next_value in intersection:
        ...
        findingPandigitalPrimeSets(value=next_value, curr_set=curr_set.copy(), intersection=next_intersection)
